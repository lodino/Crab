...
if dataset == 'hmda':
    if scenario == 7:
        X_train, X_test, y_train, y_test = load(dataset, subarea='Calcasieu')
    elif scenario == 8:
        X_train, X_test, y_train, y_test = load(dataset, subarea='Orleans')
    else:
...
if (dataset == 'hmda') and (scenario < 7):
    epoch_num = 300
...
if '0' in args.algo:
    ...
    # print(f"{clf_name} time:", time.time() - t0)
    t0 = time.time()
    clf = train_clf(clf, info_dict)
...
if '1' in args.algo:
    ...
    t0 = time.time()
    clf = train_clf(clf, info_dict)
    print(f"{clf_name + '_Reg_Bound'} time:", time.time() - t0)
...
if '3' in args.algo:
    ...
    # for s in range(2):
    #     s_idx = X_test_orig[X_test_orig[sens_attr] == s].index.intersection(
    #         get_A_idx(X_test_orig, y_test, A, A_val))
    #     for v in np.sort(get_attr(X_train_orig_sampled, y_train_sampled, parent_1).unique()):
    #         for u in np.sort(get_attr(X_train_orig_sampled, y_train_sampled, parent_2).unique()):
    #             for w in np.sort(get_attr(X_train_orig_sampled, y_train_sampled, parent_3).unique()):
    #                 idx = X_test_orig[(get_attr(X_test_orig, y_test, parent_1) == v) & (
    #                         get_attr(X_test_orig, y_test, parent_2) == u) & (
    #                                            get_attr(X_test_orig, y_test, parent_3) == w) & (
    #                                            get_attr(X_test_orig, y_test, sens_attr) == s)].index
    #                 idx = idx.intersection(get_A_idx(X_test_orig, y_test, A, A_val))
    #                 if len(idx) > 0:
    #                     weights[s].append(len(idx) / len(s_idx))
